	...
	function FlightIndicator( placeholder, type, options ) {
		...
		placeholder.each(function(){
			switch(type){
				...
				break;
				case 'airspeed':
					var html = '<div class="instrument airspeed"><img src="' + settings.img_directory + 'fi_box.svg" class="background box" alt="" /><img src="' + settings.img_directory + 'speed_mechanics.png" class="box" alt="" /><div class="speed box"><img src="' + settings.img_directory + 'fi_needle.png" style="" class="box" alt="" /></div><div class="mechanics box"></div></div>';
			...
			doc.querySelector('div.instrument').style.height=settings.size;
			doc.querySelector('div.instrument').style.width=settings.size;
			
			if(!settings.showBox){
				doc.querySelector('div.instrument img.box.background').style.display='none';
			}
		});
		...
		function _setPitch(pitch){
			...
			placeholder.each(function(){
				$(this)[0].find('div.instrument.attitude div.roll div.pitch').css('top', pitch*0.7 + '%');
			});
		}
		...
		function _setAirSpeed(speed){
			...
			speed = -18+speed;
			placeholder.each(function(){
				var node = doc.querySelector('div.instrument.airspeed div.speed');
				node.style.transform='rotate(' + speed + 'deg)';
			});	
		}
